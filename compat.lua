...
local function GetActionButton_ElvUI(action)
    for b = 1, #ELVUI_BARS do
        for i = 1, 12 do
            ...
            if button then
                local slot =
                    tonumber(button._state_action) or tonumber(button.action) or
...
-- Bartender4
local function GetActionButton_Bartender4(action)
    for i = 1, 120 do
        local button = _G["BT4Button" .. i]
        if button and button.action == action then
            return button
        end
    end
end
...
local function Flyout_HandleCompatEvent(self, event, arg1)
    ...
    if IsAddOnLoaded("ElvUI") then
        Flyout.GetActionButton_Custom = GetActionButton_ElvUI
    end

    if IsAddOnLoaded("Bongos") and IsAddOnLoaded("Bongos_ActionBar") then
        Flyout.GetActionButton_Custom = GetActionButton_Bongos
    end

    if IsAddOnLoaded("pfUI") then
        Flyout.GetActionButton_Custom = GetActionButton_PF
    end

    if IsAddOnLoaded("Bartender4") then
        Flyout.GetActionButton_Custom = GetActionButton_Bartender4
    end

    if IsAddOnLoaded("-Dragonflight3") then
        Flyout.GetActionButton_Custom = GetActionButton_Dragonflight3
    end
end
