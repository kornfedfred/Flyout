...
function Flyout.GetBagPosition(name)
...
local function UpdateBarButton(slot)
    ...
    if button then
        ...
        if not strfind(body, "/flyout", 1, true) then
            return
        end
...
local function Flyout_HandleCoreEvent(self, event, arg1, arg2, arg3, arg4, arg5)
    if event == "VARIABLES_LOADED" then
        if not Flyout_Config or (Flyout_Config["REVISION"] == nil or Flyout_Config["REVISION"] ~= revision) then
            local oldConfig = Flyout_Config or {}
            Flyout_Config = {}
            -- Migrate existing settings so users don't lose their preferences on upgrade.
            for key, value in pairs(oldConfig) do
                Flyout_Config[key] = value
            end
        end
        -- Initialize defaults if not present.
        for key, value in pairs(Flyout.DEFAULT_CONFIG) do
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        for i = 1, MAX_FLYOUT_BUTTONS do
            ...
            if not b then
                ...
                b:SetID(i)
                b:SetScript("PostClick", Flyout.OnClick)
                b:Hide()
            end
        end
        ...
        delayFrame:SetScript(
            ...
            function(self, elapsed)
                ...
                if delayElapsed >= 3 then
                    ...
                    Flyout.BuildButtonCache()
                    Flyout.UpdateBars()
                end
            end
        )
    elseif event == "ACTIONBAR_SLOT_CHANGED" then
        Flyout.Hide(true) -- Keep sticky menus open.
    ...
    for rawLine in string.gmatch(body .. "\n", "([^\n]*)\n") do
        ...
        if strfind(line, "/flyout") then
            if not lockOrder then
                ...
                if as and bs then
                    line =
                        string.sub(line, 1, as - 1) ..
                        newAction .. string.sub(line, ae + 1, bs - 1) .. oldAction .. string.sub(line, be + 1)
                end
            end
...
function Flyout.OnClick(button, mouseButton)
    ...
    if mouseButton == "RightButton" and button.flyoutParent then
        ...
        if oldAction ~= newAction then
            local slot =
                parent.flyoutSlot or tonumber(parent.action) or tonumber(parent._state_action) or
                (ActionButton_CalculateAction and ActionButton_CalculateAction(parent))
...
local function IsCurrentCast(spellIndex, bookType)
    ...
    if currentSpell == spellName then
        return true
    end
    local channelSpell = UnitChannelInfo("player")
    if channelSpell == spellName then
        return true
    end
    return false
end
...
function Flyout.HideFlyoutButtons(keepOpenIfSticky)
    ...
    -- Restore arrow to original strata (it was moved to FULLSCREEN in Flyout.Show())
    if Flyout.lastArrow then
        Flyout.lastArrow:SetFrameStrata(Flyout.lastArrow.flyoutOriginalStrata)
        Flyout.lastArrow = nil
    end
...
function Flyout.UpdateVisibleCooldowns()
    for i = 1, 12 do
        local b = _G["FlyoutButton" .. i]
...
function Flyout.Show(button)
    ...
    local arrow = _G[button:GetName() .. "FlyoutArrow"]
    if arrow then
        if Flyout.lastArrow and Flyout.lastArrow ~= arrow then
            Flyout.lastArrow:SetFrameStrata(Flyout.lastArrow.flyoutOriginalStrata)
        end
        Flyout.lastArrow = arrow
    ...
    for i, n in ipairs(button.flyoutActions) do
        ...
        if texture then
            ...
            b:SetBackdropColor(
                Flyout_Config["BORDER_COLOR"][1],
                Flyout_Config["BORDER_COLOR"][2],
                Flyout_Config["BORDER_COLOR"][3]
            )
            if InCombatLockdown() then
                break
            end
...
function Flyout.BuildButtonCache()
    wipe(buttonCache)

    if Flyout.GetActionButton_Custom then
        for slot = 1, 120 do
            local button = Flyout.GetActionButton_Custom(slot)
            if button then
    ...
    for i = 1, #bars do
        for j = 1, 12 do
            ...
            if button and button:IsVisible() then
                local slot =
                    tonumber(button.action) or tonumber(button._state_action) or
                    (ActionButton_CalculateAction and ActionButton_CalculateAction(button))
...
function Flyout.GetActionButton(action)
    ...
    if Flyout.GetActionButton_Custom then
        local button = Flyout.GetActionButton_Custom(action)
        if button then
            return button
        end
    end
